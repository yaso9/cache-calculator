<script lang="ts">
  ...
  let setIndex: string;
  let tag: string;
  let recSetIdx: string;
  let recTag: string;
  let recBlockAddr: string;
  let recMinAddr: string;
  let recMaxAddr: string;
  ...
  $: {
    ...
    recBlockAddr = (
      (parseInt(recTag, 16) << s) |
      parseInt(recSetIdx, 16)
    ).toString(16);
    recMinAddr = (parseInt(recBlockAddr, 16) << b).toString(16);
    recMaxAddr = (parseInt(recMinAddr, 16) + B - 1).toString(16);
  }
</script>
    ...
    <div class="card col-6 offset-3">
      <div class="card-body">
        ...
        <div class="alert alert-primary" role="alert">
          Recall, direct mapped means A = 1, fully associative means S = 1, and
          set associative means everything else. The notation we use is $(A, B,
          C).
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
          <label for="b" class="form-label"
            >b (# block offset bits) = log(B)</label
          >
          <input
            type="number"
            class="form-control"
        ...
        <div class="input-group mb-3">
          <label for="rec-set-idx" class="form-label">Set Index</label>
          <div class="input-group">
            <span class="input-group-text">0x</span>
            <input
              type="text"
              class="form-control"
              id="rec-set-idx"
              bind:value={recSetIdx}
            />
          </div>
        </div>
        <div class="input-group mb-3">
          <label for="rec-tag" class="form-label">Tag</label>
          <div class="input-group">
            <span class="input-group-text">0x</span>
            <input
              type="text"
        ...
        <div class="input-group mb-3">
          <label for="rec-block-addr" class="form-label"
            >Recreated Block Address</label
          >
          <div class="input-group">
            <span class="input-group-text">0x</span>
            <input
        ...
        <div class="input-group mb-3">
          <label class="form-label"
            >Recreated Memory Addresses (inclusive)</label
          >
          <div class="input-group">
            <span class="input-group-text">0x</span>
            <input
